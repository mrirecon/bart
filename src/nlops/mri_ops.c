...
static void sense_model_set_data_noncart_fun(const nlop_data_t* _data, int Narg, complex float* args[Narg])
{
	...
	struct vptr_hint_s* hint = hint_delayed_create(bart_delayed_loop_flags);
	complex float* vtraj = vptr_alloc(d->N, d->model->config->trj_dims, CFL_SIZE, hint);
#ifdef USE_CUDA
	...
	nufft_update_traj(d->model->nufft, d->N, d->model->config->trj_dims, vtraj, d->model->config->pat_dims, pattern, d->model->config->bas_dims, d->model->config->basis);

	if (NULL != d->model->nufft_loss)
		nufft_update_traj(d->model->nufft_loss, d->N, d->model->config->trj_dims, vtraj, MD_SINGLETON_DIMS(d->N), NULL, d->model->config->bas_dims, d->model->config->basis);
...
static const struct nlop_s* nlop_mri_loss_create_s(bool fft, struct sense_model_s* model)
{
	...
	struct vptr_hint_s* hint = hint_delayed_create(bart_delayed_loop_flags);
	const struct nlop_s* ret = nlop_from_linop_F(linop_vptr_wrapper(hint, (struct linop_s*)lop_sense));
	vptr_hint_free(hint);
