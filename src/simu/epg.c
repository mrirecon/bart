 ...
 *
 * and computes partial derivatives similiar to K. Layton, M. Morelande, D. Wright, P. Farrell, B. Moran, L A. Johnston. Modelling and Estimation of Multicomponent T2 Distributions, IEEE TMI, 2013
 */
...
void create_rf_pulse_matrix_der(complex float T[3][3], complex float dT[4][3][3], float alpha, float phi)
{
	...
	float ca = cos(alpha);
	float sa = sin(alpha);
	complex float i_phi = 0.0 + I*phi;  
	complex float i_half = 0.0 + I*0.5;  

	T[0][0] = cah2;
	T[0][1] = cexpf(2.0*i_phi)*sah2;
	T[0][2] = -1.0*I*cexpf(i_phi)*sa;

	T[1][0] = cexpf(-2.0 * i_phi)*sah2;
	T[1][1] = cah2;
	T[1][2] = I * cexpf(-i_phi)*sa;
	...
	if (NULL != dT) {
		...
		dT[2][0][2] = -1.0 * I * cexpf(i_phi) * ca * alpha;

		dT[2][1][0] = cexpf(-2.0 * i_phi) * sah * cah * alpha;
		dT[2][1][1] = -cah * sah * alpha;
		dT[2][1][2] = 1.0 * I * cexpf(-i_phi) * ca * alpha;
		 ...
		 float adc_phase)
{
	...
	for (int j = 0; j < 3; j++)
	    for (int k = 0; k < M; k++)
		for (int m = 0; m < 4; m++)
		    dstates[m][j][k][idx_current] =
			dstate_current[m][j][k] * phasor_offset;
}
...
void epg_adc(int idx_current, int N, int M, complex float signal[N],
		 ...
		 float adc_phase)
{
    epg_adc_der(idx_current, N, M, signal, states, NULL, NULL,
		state_current, NULL, adc_phase);
}
...
void cpmg_epg_der(int N, int M, complex float signal[N], complex float states[3][M][N], complex float dsignal[4][N], complex float dstates[4][3][M][N], float rf_exc, float rf_ref, float TE, float T1, float T2, float B1, float offres)
{
	...
	complex float dT_ref[4][3][3] = {{{0.0}}};
...
{
	if (NULL != states)
		memset(states, 0.0, CFL_SIZE*3*M*N);
