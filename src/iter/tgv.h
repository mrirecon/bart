...
extern struct reg tv_reg(unsigned long flags, unsigned long jflags, float lambda, int N, const long img_dims[N]);
extern struct reg2 tgv_reg(unsigned long flags, unsigned long jflags, float lambda, int N, const long in_dims[N], long isize, long* ext_shift);
