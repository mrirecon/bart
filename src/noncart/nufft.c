...
static void apply_linphases_3D_int(vptr_fun_data_t* _data, int N, int D, const long* dims[N], const long* strs[N], void* args[N])
{
	...
	if (D != md_calc_blockdim(D, img_dims, img_strs, CFL_SIZE)) {

		unsigned long flags = (MD_BIT(D) - 1) & ~(MD_BIT(md_calc_blockdim(D, img_dims, img_strs, CFL_SIZE)) - 1);
		...
		const long** ndims_p = (const long**)ndims;
		const long** nstrs_p = (const long**)strs;
...
static void toeplitz_mult(const struct nufft_data* data, complex float* dst, const complex float* src)
{
	...
	if (!md_check_equal_dims(data->N, data->cmT_dims, data->cml_dims, ~0UL)) {
		...
		if (data->conf.real) {
			...
			if (data->conf.upper_triag)
				md_tenmul_upper_triag2(5, 6, ND + 1, MD_REAL_DIMS(ND, mdims), MD_REAL_STRS(ND, cmT_strs, FL_SIZE), (float*)gridT, MD_REAL_STRS(ND, data->cml_strs, FL_SIZE), (float*)grid, MD_REAL_DIMS(ND, data->psf_dims), MD_REAL_STRS(ND, data->psf_strs, 0), (float*)psf);
			else
