		...
		const struct linop_s** fft_opp, unsigned long shared_img_dims, unsigned long lowmem_stack)
{
	...
	for (int i = DIMS - 1; i > MAPS_DIM; i--) {
		...
		const struct linop_s* lop = sense_nc_init(n_max_dims, n_map_dims, maps, n_ksp_dims,
				n_traj_dims, traj, _conf, n_wgs_dims, weights,
				n_basis_dims, basis, NULL, shared_img_dims, lowmem_stack);

		for (int j = 1; j < max_dims[i]; j++) {

			auto tmp = sense_nc_init(n_max_dims, n_map_dims, maps + j * offset_sens, n_ksp_dims, n_traj_dims,
						traj + j * offset_traj, _conf, n_wgs_dims, weights + j * offset_weights,
