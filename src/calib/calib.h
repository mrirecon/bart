...
extern void calib(const struct ecalib_conf* conf, const long out_dims[DIMS], _Complex float* out_data, _Complex float* eptr, 
			unsigned int SN, float svals[SN], const long calreg_dims[DIMS], const _Complex float* calreg_data);

extern void calib2(const struct ecalib_conf* conf, const long out_dims[DIMS], _Complex float* out_data, _Complex float* eptr, unsigned int SN, float svals[SN], const long calreg_dims[DIMS], const _Complex float* data, const long msk_dims[3], const _Bool* msk);
...
extern void compute_kernels(const struct ecalib_conf* conf, long nskerns_dims[5], _Complex float** nskerns_ptr, unsigned int SN, float svals[SN], const long caldims[DIMS], const _Complex float* caldata);
