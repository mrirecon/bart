...
static void wavelet_forward(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	...
	fwt2(data->N, data->flags, shifts, data->odims, data->ostr, dst, data->idims, data->istr, src, data->minsize, 4, wavelet3_dau2);
}
...
static void wavelet_adjoint(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	...
	iwt2(data->N, data->flags, shifts, data->idims, data->istr, dst, data->odims, data->ostr, src, data->minsize, 4, wavelet3_dau2);
}
