...
static void reduce_sum_int(vptr_fun_data_t* d, int N, int D, const long* dims[N], const long* strs[N], void* args[N])
{
	...
	if (CAST_DOWN(vptr_mpi_reduce_s, d)->use_double)
		mpi_reduce_sumD_kernel(tot, args[0] + md_calc_offset(N, strs[0], pos));
	else
		mpi_reduce_sum_kernel(tot, args[0] + md_calc_offset(N, strs[0], pos));
#else
		(void)size;
		(void)args;
		(void)d;
...
void mpi_reduce_sum(int N, const long dims[N], float* ptr)
{
	...
	exec_vptr_fun_gen(reduce_sum_int, CAST_UP(PTR_PASS(_d)), 1, N, ~0UL, MD_BIT(0), MD_BIT(0), &dims, (const long*[1]) { MD_STRIDES(N, dims, FL_SIZE) }, (void*[1]) { ptr }, (size_t[1]){ FL_SIZE }, false);
}

void mpi_reduce_zsum(int N, const long dims[N], complex float* ptr)
{
	PTR_ALLOC(struct vptr_mpi_reduce_s, _d);
	SET_TYPEID(vptr_mpi_reduce_s, _d);
	_d->INTERFACE.del = NULL;
	...
	exec_vptr_fun_gen(reduce_sum_int, CAST_UP(PTR_PASS(_d)), 1, N, ~0UL, MD_BIT(0), MD_BIT(0), &dims, (const long*[1]) { MD_STRIDES(N, dims, CFL_SIZE) }, (void*[1]) { ptr }, (size_t[1]){ CFL_SIZE }, false);
...
void mpi_reduce_sumD(int N, const long dims[N], double* ptr)
{
	PTR_ALLOC(struct vptr_mpi_reduce_s, _d);
	SET_TYPEID(vptr_mpi_reduce_s, _d);
	_d->INTERFACE.del = NULL;
	...
	exec_vptr_fun_gen(reduce_sum_int, CAST_UP(PTR_PASS(_d)), 1, N, ~0UL, MD_BIT(0), MD_BIT(0), &dims, (const long*[1]) { MD_STRIDES(N, dims, DL_SIZE) }, (void*[1]) { ptr }, (size_t[1]){ DL_SIZE }, false);
}
void mpi_reduce_zsumD(int N, const long dims[N], complex double* ptr)
{
	PTR_ALLOC(struct vptr_mpi_reduce_s, _d);
	SET_TYPEID(vptr_mpi_reduce_s, _d);
	_d->INTERFACE.del = NULL;
	...
	exec_vptr_fun_gen(reduce_sum_int, CAST_UP(PTR_PASS(_d)), 1, N, ~0UL, MD_BIT(0), MD_BIT(0), &dims, (const long*[1]) { MD_STRIDES(N, dims, CDL_SIZE) }, (void*[1]) { ptr }, (size_t[1]){ CDL_SIZE }, false);
}
