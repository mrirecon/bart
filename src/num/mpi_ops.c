...
{
	int ND = md_calc_blockdim(D, dims[0], strs[0], (CAST_DOWN(vptr_mpi_reduce_s, d)->use_double) ? DL_SIZE : FL_SIZE);

	long size = md_calc_size(ND, dims[0]);
	...
	if (CAST_DOWN(vptr_mpi_reduce_s, d)->use_double)
		mpi_reduce_sumD_kernel(size, args[0] + md_calc_offset(N, strs[0], pos));
	else
		mpi_reduce_sum_kernel(size, args[0] + md_calc_offset(N, strs[0], pos));
#else
		(void)size;
		(void)args;
		(void)d;
...
{
	mpi_reduce_sum(N + 1, MD_REAL_DIMS(N, dims), (float*)ptr);
}
...
{
	mpi_reduce_sumD(N + 1, MD_REAL_DIMS(N, dims), (double*)ptr);
}
