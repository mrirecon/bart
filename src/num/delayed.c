...
#define BITMASK(b) (1UL << ((int)(b) % (int)sizeof(unsigned long)))
#define BITSLOT(b) ((int)(b) / (int)sizeof(unsigned long))
...
static const char* print_ptr(delayed_op_t* op)
{
	...
	for (int i = 0; i < op->N; i++) {
		...
		ret = ptr_append_printf(ret, "%s%s", ptr, (i < op->N - 1) ? ", " : ")");
		xfree(ptr);
	}
...
static const char* print_delayed_fun_f(delayed_op_t* op, bool nested)
{
	...
	if (NULL != op->debug) {
		...
		const char* ret2 = ptr_printf("%s %s", op->TYPEID->name, ptr);
		xfree(ptr);
		ret = ptr_append_print_dims(ret2, op->D, op->ldims);
	}
...
static const char* delayed_op_alloc_free_debug(delayed_op_t* op, bool /*nested*/)
{
	...
	const char* ret2 = ptr_printf("%s %s", op->TYPEID->name, ptr);
	xfree(ptr);
	ret = ptr_append_print_dims(ret2, op->args[0].N - 1, op->args[0].adims + 1);
	...
	ret = ptr_append_printf(ret, " loop: (");

	for (int i = 0; i < op->D; i++)
		if (1 < op->ldims[i])
			ret = ptr_append_printf(ret, "%d, ", i);

	ret = ptr_append_printf(ret, ")");
...
{
	vptr_free(CAST_DOWN(delayed_free_s, op)->INTERFACE.args[0].ptr);
}
...
//	case offsetof(struct vec_ops, swap) : name = "swap"; break;
	...
	xfree(ptr);
	return ptr_append_print_dims(ret, op->D, op->ldims);
}
...
bool delayed_queue_make_op(enum delayed_md_fun_type type, size_t offset, int D, const long dim[D], int N, const long* strs[N], const void* ptr[N], const size_t sizes[N])
{
	...
	if (   (offset == offsetof(struct vec_ops, fmac))
		...
		BITSET(op->INTERFACE.read_flags, 0);
...
static const char* delayed_slice_debug(delayed_op_t* op, bool nested)
{
	...
	ret = ptr_append_print_dims(ret, op->D, op->ldims);
...
static const char* delayed_chain_debug(delayed_op_t* _op, bool nested)
{
	...
	ret = ptr_append_print_dims(ret, _op->D, _op->ldims);
