...
extern void svd(long M, long N, complex float U[M][M], complex float VT[N][N], float S[(N > M) ? M : N], complex float A[M][N]);
...
extern void svd_double(long M, long N, complex double U[M][M], complex double VT[N][N], double S[(N > M) ? M : N], complex double A[M][N]);
