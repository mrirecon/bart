...
static void fft_cache_init(void)
{
	if (NULL != fft_cache)
		return;
	
#pragma omp critical(bart_fftcahce)
	if (NULL == fft_cache)
...
const struct operator_s* fft_create2(int D, const long dimensions[D], unsigned long flags, const long ostrides[D], const complex float* dst, const long istrides[D], const complex float* src, bool backwards)
{
	...
	const struct operator_s* op = search(D, dimensions, flags, ostrides, istrides, backwards, inplace, measure);
