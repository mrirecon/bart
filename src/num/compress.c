...
long md_compress_sphere_to_index(long N, const long dims[N], long* index, float r)
{
	long str[N];
	md_calc_strides(N, str, dims, sizeof(long));
	...
	do {
		long sr = 0;
		for (int i = 0; i < N; i++)
			sr += (pos[i] - dims[i] / 2) * (pos[i] - dims[i] / 2);

		if (sr <= r * r)
			MD_ACCESS(N, str, pos, index) = (max++);
		else
			MD_ACCESS(N, str, pos, index) = -1;
