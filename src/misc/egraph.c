...
{
	const char* nodes = ptr_printf("");
	const char* edges = ptr_printf("");
	bool sort = true;
	...
	for (int i = 0; i < list_count(graph); i++) {
		...
		nodes = ptr_append_printf(nodes, "\nnode_%p", node);

		if (NULL != node->name)
			nodes = ptr_append_printf(nodes, " [label=\"%s\"]", node->name);

		if (sort && NULL != last_node)
			edges = ptr_append_printf(edges, "\n%s -> node_%p [style=invis]", last_node, node);
