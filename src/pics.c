...
int main_pics(int argc, char* argv[argc])
{
	...
	if (NULL != basis_file) {
		...
		assert(!MD_IS_SET(bart_mpi_split_flags, TE_FLAG));
	...
	struct iter it = italgo_config(algo, nr_penalties, ropts.regs, maxiter, step, eigen ? 20 : 0, hogwild, admm, fista, scaling, NULL != image_truth);
