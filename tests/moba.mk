...
tests/test-moba-bloch-irflash-psf: phantom signal fft ones index scale moba slice fmac nrmse
	...
	$(TOOLDIR)/index 5 100 tmp1.ra   						;\
	$(TOOLDIR)/scale 0.005 tmp1.ra TI.ra                    	       		;\
	$(TOOLDIR)/moba --bloch --sim STM --seq IR-FLASH,TR=0.005,TE=0.003,FA=6,Trf=0.00001,BWTP=4,pinv,ipl=0,ppl=0 --other pscale=1:1:1:1,pinit=3:1:1:0 -i11 -C300 -s0.95 -R3 -f1 -o1 -j0.001 --scale_data=5000. --scale_psf=1000. --normalize_scaling -p psf.ra k_space.ra TI.ra reco.ra	;\
...
tests/test-moba-bloch-irflash-traj: traj repmat phantom signal fmac ones scale index moba slice spow fmac resize nrmse
	...
	$(TOOLDIR)/scale 0.005 tmp1.ra TI.ra                    	       		;\
	$(TOOLDIR)/moba --bloch --sim STM --seq IR-FLASH,TR=0.005,TE=0.003,FA=6,Trf=0.00001,BWTP=4,pinv,ipl=0,ppl=0 --other pscale=1:1:1:1,pinit=3:1:1:0 -i11 -C300 -s0.95 -d3 -R3 -o1 -j0.001 --scale_data=5000. --scale_psf=1000. --normalize_scaling -t traj.ra k_space.ra TI.ra reco.ra sens.ra	;\
	$(TOOLDIR)/slice 6 0 reco.ra r1map.ra						;\
...
tests/test-moba-bloch-irflash-traj-fixfa: traj repmat phantom signal fmac scale index moba slice spow fmac resize nrmse ones
	...
	$(TOOLDIR)/index 5 1000 tmp1.ra   						;\
	$(TOOLDIR)/scale 0.005 tmp1.ra TI.ra                    	       		;\
	$(TOOLDIR)/moba --bloch --sim STM --seq IR-FLASH,TR=0.005,TE=0.003,FA=8,Trf=0.00001,BWTP=4,pinv,ipl=0,ppl=0 --other pscale=1:1:1:0,pinit=3:1:1:0 -i11 -C30 -s0.95 -R3 -o1 -j0.001 --scale_data=5000. --scale_psf=1000. --normalize_scaling -t traj.ra k_space.ra TI.ra reco.ra sens.ra	;\
	$(TOOLDIR)/slice 6 0 reco.ra r1map.ra						;\
	$(TOOLDIR)/spow -- -1. r1map.ra t1map.ra						;\
	$(TOOLDIR)/phantom -x8 -c circ2.ra						;\
	$(TOOLDIR)/resize -c 0 16 1 16 circ2.ra circ.ra					;\
...
tests/test-moba-bloch-irbssfp-psf: traj repmat phantom signal fmac fft ones index moba spow slice scale resize nrmse
	...
	$(TOOLDIR)/moba --bloch --sim STM --seq IR-BSSFP,TR=0.0045,TE=0.00225,FA=45,Trf=0.00001,BWTP=4,pinv,ipl=0,ppl=0.00225 --other pscale=1:1:1:0,pinit=3:1:1:0 -i11 -C300 -s0.95 -R3 -o1 -j0.001 --scale_data=5000. --scale_psf=1000. --normalize_scaling -p psf.ra k_space.ra dummy_ti.ra reco.ra sens.ra	;\
	$(TOOLDIR)/slice 6 0 reco.ra r1map.ra				;\
...
tests/test-moba-ir-meco-traj: traj reshape scale phantom signal extract slice transpose fmac index ones saxpy moba resize looklocker nrmse
	...
	$(TOOLDIR)/moba -i20 -d4 -D -m9 -R3 -o1.25 -C400 -k --kfilter-2 --normalize_scaling --scale_data 500 --scale_psf 500 -B0. --other pinit=1:1:1:1:1:1:0.05:0.05,pscale=1:1:1:1:1:1:0.05:0.05,echo=out_TE.ra -b 1:1 -t out_traj.ra data_0.ra out_TI.ra reco.ra sens.ra ;\
...
tests/test-moba-ir-meco-5para-traj: traj reshape scale phantom signal extract slice transpose fmac index ones saxpy moba resize looklocker nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)	               		 	;\
	$(TOOLDIR)/traj -x16 -y9 -t30 -r -D -E -G -s2 -e7 -c _traj.ra                  	;\
	$(TOOLDIR)/reshape 1028 270 1 _traj.ra traj2.ra					;\
	$(TOOLDIR)/scale 0.5 traj2.ra traj.ra						;\
	$(TOOLDIR)/phantom -k -c -t traj.ra basis_geom.ra				;\
	$(TOOLDIR)/signal -I -C -r0.0127 -e0.0016 -n2160 -m8 -d0. -1 1.25:1.25:1 -2 0.05:0.05:1 -0 30:30:1 -4 0.3:0.3:1 signal.ra	;\
	$(TOOLDIR)/reshape 48 270 8 signal.ra tmp_signal_all_1.ra      			;\
	$(TOOLDIR)/extract 5 1 8 tmp_signal_all_1.ra tmp_signal_all_2.ra                ;\
	$(TOOLDIR)/transpose 4 2 tmp_signal_all_2.ra tmp_signal_all_3.ra                ;\
	$(TOOLDIR)/fmac -s 64 basis_geom.ra tmp_signal_all_3.ra data.ra                 ;\
	...
	$(TOOLDIR)/ones 6 1 1 1 1 1 54 tmp1.ra 						;\
	$(TOOLDIR)/saxpy 0.0254 tmp1.ra tmp2.ra out_TI.ra				;\
	$(TOOLDIR)/moba -i20 -d4 -D -m8 -R3 -o1.25 -C400 -k --kfilter-2 --normalize_scaling --scale_data 500 --scale_psf 500 -B0. --other pinit=1:1:1:0.05:0.05,pscale=1:1:1:0.05:0.05,echo=out_TE.ra -b 1:1 -t out_traj.ra data_0.ra out_TI.ra reco.ra sens.ra ;\
	$(TOOLDIR)/phantom -x 8 -c circ.ra						;\
	$(TOOLDIR)/resize -c 0 8 1 8 reco.ra reco_maps.ra				;\
	$(TOOLDIR)/extract 6 0 3 reco_maps.ra reco_w_maps.ra				;\
	$(TOOLDIR)/looklocker -t0. -D0. reco_w_maps.ra reco_w_t1.ra			;\
	...
	$(TOOLDIR)/scale -- 1.25 circ.ra ref.ra						;\
	$(TOOLDIR)/nrmse -t 0.003 ref.ra reco_w_t1_masked.ra				;\
	$(TOOLDIR)/slice 6 3 reco_maps.ra reco_r2s2.ra					;\
	$(TOOLDIR)/fmac circ.ra reco_r2s2.ra reco_r2s2_masked.ra			;\
	$(TOOLDIR)/scale -- 20 circ.ra ref.ra						;\
	$(TOOLDIR)/nrmse -t 0.02 ref.ra reco_r2s2_masked.ra				;\
...
TESTS_SLOW += tests/test-moba-ir-meco-traj tests/test-moba-ir-meco-5para-traj
