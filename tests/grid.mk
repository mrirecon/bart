...
$(TESTS_OUT)/grid_t.ra: traj
	$(TOOLDIR)/traj -x 128 -y 129 $@

tests/test-grid_traj: grid extract nrmse $(TESTS_OUT)/grid_t.ra
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)		;\
	$(TOOLDIR)/grid -t $(TESTS_OUT)/grid_t.ra -T 7 grid_tt	;\
	$(TOOLDIR)/extract 0 0 3 10 0 1 grid_tt grid_tt_e	;\
	$(TOOLDIR)/nrmse -t 0 $(TESTS_OUT)/grid_t.ra grid_tt_e	;\
	rm *.cfl *.hdr; cd .. ; rmdir $(TESTS_TMP)
	touch $@

tests/test-grid_cart-traj: traj grid extract nrmse $(TESTS_OUT)/grid_t.ra
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)		;\
	$(TOOLDIR)/grid -k -D 128:129 grid_k			;\
	$(TOOLDIR)/extract 0 0 3 grid_k grid_k_e		;\
	$(TOOLDIR)/nrmse -t 0 $(TESTS_OUT)/grid_t.ra grid_k_e	;\
	rm *.cfl *.hdr; cd .. ; rmdir $(TESTS_TMP)
	touch $@
...
tests/test-grid_traj_compat: grid nrmse $(TESTS_OUT)/grid_t.ra
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)		;\
	$(TOOLDIR)/grid -t $(TESTS_OUT)/grid_t.ra -T 7 traj	;\
