...
tests/test-seq-raga: seq traj extract nrmse 
	...
	$(TOOLDIR)/traj -x 512 -y 377 -r -D trj_ref.ra 	;\
...
tests/test-seq-raga-chrono: seq traj extract nrmse 
	...
	$(TOOLDIR)/seq -r 377 -t377 --raga --chrono grad.ra mom.ra samples.ra 		;\
	$(TOOLDIR)/extract 0 2 5 samples.ra trj_seq.ra				;\
	$(TOOLDIR)/nrmse -t 2e-7 trj_ref.ra trj_seq.ra				;\
	rm *.ra ; cd .. ; rmdir $(TESTS_TMP)
	touch $@
...
tests/test-seq-raga-sms: seq traj extract nrmse
	...
	$(TOOLDIR)/seq -r 377 -t377 --raga --chrono --mb_factor=3 -m3 grad.ra mom.ra samples.ra	;\
	$(TOOLDIR)/extract 0 2 5 samples.ra trj_seq.ra					;\
...
tests/test-seq-raga-sms-al: seq traj extract nrmse
	...
	$(TOOLDIR)/seq -r 377 -t377 --raga_al --chrono --mb_factor=3 -m3 grad.ra mom.ra samples.ra	;\
	$(TOOLDIR)/extract 0 2 5 samples.ra trj_seq.ra						;\
	$(TOOLDIR)/nrmse -t 2e-7 trj_ref.ra trj_seq.ra						;\
	rm *.ra ; cd .. ; rmdir $(TESTS_TMP)
	touch $@
...
tests/test-seq-offcenter: seq traj extract scale phantom fovshift fmac nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)							;\
	$(TOOLDIR)/traj -x 512 -y 377 -r -A -s 1 --double-base trj_ref.ra				;\
	$(TOOLDIR)/seq -s 25.6:12.8:0 -r 377 -t377 --raga --chrono --no-spoiling grad.ra mom.ra samples.ra	;\
	$(TOOLDIR)/extract 0 2 5 samples.ra trj_seq.ra							;\
...
tests/test-seq-raga-ordering: seq traj extract raga bin nrmse 
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/seq -r 377 -t377 --raga grad.ra mom.ra samples.ra 				;\
	$(TOOLDIR)/seq -r 377 -t377 --raga --chrono grad_c.ra mom_c.ra samples_c.ra 	;\
