...
static bool test_affine_to_grid(void)
{
	...
	nlop_rot = nlop_affine_to_grid_F(nlop_rot, dims, dims);
	...
	UT_RETURN_ASSERT(UT_TOL > md_znrmse(1, MD_DIMS(12), id, trafo));
}

UT_REGISTER_TEST(test_affine_to_grid);
...
UT_REGISTER_TEST(test_affine_rot_transpose);
...
UT_REGISTER_TEST(test_affine_nlop_rot2D);
...
UT_REGISTER_TEST(test_affine_nlop_interpolate);
...
UT_REGISTER_TEST(test_affine_nlop_interpolate_coord);
...
UT_REGISTER_TEST(test_affine_nlop_interpolate_cood_points);
...
UT_REGISTER_TEST(test_affine_nlop_interpolate_cood_points_keys);
