...
static bool rf_spoiling_spoiled(void)
{
	...
	do {
		...
		if (fabs(ref[i] - rf_spoiling(DIMS, seq_state.pos, &seq)) > UT_TOL)
			return false;
...
static bool test_sinc(void)
{
	...
	const double good_norm = 330.154932; // expected in reference implementation
	...
	double good[2] = { 0.000445, 0.046478 }; // expected in reference implementation
	...
	for (int i = 0; i < rf_shape[0].samples; i++) {
		...
		if (((i < 294) || (i > 946)) && (fabs(shape_pha[i] - M_PI) > 1e-4))
			return false;
		else if ((i > 294) && (i < 946) && (fabs(shape_pha[i]) > 1e-5))
			return  false;
	}
...
static bool test_sms(void)
{
	...
	const double good_norm = 110.051648; // expected in reference implementation
	...
	float mag[3][7] = {{ 1., 0.349166, 0.349166, 0.243029, 0.243031, 0.199427, 0.199427 }, 
			   { 0., 0.250700, 0.889000, 0.188331, 0.836786, 0.473933, 0.344311 },
			   { 0., 0.889000, 0.250700, 0.836787, 0.188333, 0.344310, 0.473932 }};
	float pha[3][7] = {{ 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 3.141593, 3.141593 },
